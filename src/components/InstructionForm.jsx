...
import extractPageMetadata from '../utils/ExtractPageMetadata';
import useUseCase from '../hooks/useUseCase';
import { loadIteration, saveIteration } from '../utils/LocalStorage';
...
const InstructionForm = () => {
  ...
  const [iteration, setIteration] = useState(() => loadIteration(useCase));
  ...
  // whenever iteration changes, persist it
  useEffect(() => {
    saveIteration(useCase, iteration);
  }, [useCase, iteration]);
  ...
  const handleAskLLM = async e => {
    ...
    // const useCase =
    try {
      const response = await generateScriptOllama(
        promptText,
        metadata,
        useCase,
        iteration
      ...
      setGeneratedCode(response?.code || 'No valid code found!');
    ...
    <>
      <div data-ignore-metadata="true">
        ...
        {isModalOpen && (
          <div
            ref={modalRef}
            className="fixed bottom-24 right-6 w-full max-w-md h-[70vh] max-h-[600px] flex flex-col rounded-xl shadow-2xl bg-base-100 border border-base-300 z-50 overflow-hidden transform transition-all duration-500 ease-out origin-bottom-right"
            style={{
              opacity: 0,
            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
              <form onSubmit={handleAskLLM} className="mb-6">
                <div className="form-control">
                  ...
                  <div id="prompt-help" className="text-sm mt-2 text-gray-500">
              ...
              <div className="mb-6 w-full flex items-center justify-between">
                ...
                <button className="btn btn-success btn-xs">JSON</button>
                <button
